...
class BWT_IndexNow_Admin_Utils {
	...
	public static function get_submissions( $table ) {
		...
		$date = time() - 48 * 60 * 60;
			$results = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $table_name . ' WHERE submission_date > %d ORDER BY submission_date DESC LIMIT 101', $date ), OBJECT );
		if ( is_array( $results ) && count( $results ) === 101 ) {
			$ids = array();
	...
	public  static function generate_indexnow_key()
	{
		...
		update_option( 'indexnow-admin_api_key', $api_key );
